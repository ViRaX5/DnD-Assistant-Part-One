...
// The Mobile Unlock Trick (Requires them to tap a UI button, not the map)
...
socket.on('audio:syncPlay', async (data) => {
    ...
    if (currentFile !== targetFile) {
        playerAudio.pause()        // Force kill any playing ghost tracks
        playerAudio.src = data.url
        playerAudio.load()         // Forces mobile browsers to completely flush the old buffer
    }
